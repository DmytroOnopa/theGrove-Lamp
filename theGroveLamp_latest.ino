...
enum Mode { NORMAL, ANIMATION, POLICE };
...
void handleButton() {
  ...
  if (clickCount > 0 && (millis() - lastPressTime) > clickTimeout) {
    switch (clickCount) {
      case 1:
        shortBeep();
        if (currentMode == ANIMATION) currentEffect = (currentEffect + 1) % numEffects;
        else if (currentMode == POLICE) currentFlasher = (currentFlasher == POLICE_FLASHER) ? YELLOW_BLUE_FLASHER : POLICE_FLASHER;
        else changeBrightness = !changeBrightness;
        break;
      case 2:
        longBeep();
...
void discoEffect() {
  ...
  const float swirlSpeed = 0.05;
  const float hueSpeed = 1.0;
  
  unsigned long now = millis();
  
  if (now - lastUpdate > 30) {  // Оновлення кожні 30 мс
    lastUpdate = now;
    ...
    for (int i = 0; i < LED_COUNT; i++) {
      // Розрахунок позиції в спіралі
      float spiralPos = (float)i / LED_COUNT * 2.0 * PI;
      float wave = sin(angle + spiralPos * 3.0) * 0.5 + 0.5;
      
      // Зміна кольору вздовж спіралі
      uint16_t pixelHue = hue + i * 200;
      uint8_t saturation = 255;
      uint8_t value = brightness * wave * 0.8 + brightness * 0.2;
      
      // Додаємо випадкові зірки
      if (random(100) < 3) {
        value = brightness;  // Яскраві спалахи
        saturation = 200;    // Менше насичення для білих зірок
      }
      
      strip.setPixelColor(i, strip.ColorHSV(pixelHue, saturation, value));
    }
